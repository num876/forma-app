  ...
  return (
    <div className="md:hidden flex items-center">
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            {/* Backdrop - darker overlay to obscure page */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 bottom-0 w-full max-w-sm z-[999] bg-background flex flex-col overflow-y-auto"
            ...
            >
              ...
              <nav className="flex flex-col p-6 gap-6 font-black text-3xl uppercase tracking-widest font-serif flex-1">
                <Link href="/" onClick={() => setIsOpen(false)} className="border-b-[3px] border-border pb-4 hover:text-primary transition-colors">Home</Link>
                <Link href="/history" onClick={() => setIsOpen(false)} className="border-b-[3px] border-border pb-4 hover:text-primary transition-colors">History Explorer</Link>
              </nav>
